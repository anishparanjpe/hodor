...
int main(int argc, char *argv[])
{
  ...
  size_t pwdlen;
  char *passwd;
  unsigned char *passwd2 = (unsigned char *)malloc(VALSIZE);
  ...
  while (1) {
    ...
    printf("\nInput Domain: ");
    if (!fgets(input_domain, MAX_DOMAIN, fp)) break;
    strtok(input_domain, "\n");
    assert(strlen(input_domain) <= MAX_DOMAIN);
    printf("\nPassword: ");
    if (!fgets(input_passwd, MAX_PASSWD, fp)) break;
    strtok(input_passwd, "\n");
    assert(strlen(input_passwd) >= 8);
    assert(strlen(input_passwd) <= MAX_PASSWD);
  ...
  while (1) {
    ...
    printf("\nLookup Domain: ");
    if (!fgets(input_domain, 32, fp)) break;
    strtok(input_domain, "\n");
...
int make_key_from_master(char *master, unsigned char **enc_key, unsigned char **hmac_key)
{
  ...
  int i;
  unsigned int len = strlen(master), elen, hlen;
  unsigned char *enc_buf = (unsigned char *)malloc(KEYSIZE);
  ...
  for (i = 0; i <= KEYSIZE; i++)
    if (i <= len)
      *(enc_buf + i) = (master[i] + 13) % 255;
    else
      *(enc_buf + i) = '0';

  for (i = 0; i <= KEYSIZE; i++)
    if (i <= len)
      *(hmac_buf + i) = (master[i] + 5) % 255;
    else
      *(hmac_buf + i) = '0';

  digest_message(enc_buf, KEYSIZE, enc_key, &elen);
  digest_message(hmac_buf, KEYSIZE, hmac_key, &hlen);
    ...
    unsigned char *enc_key, unsigned char *hmac_key )
{
  ...
  char *choices = "!\"#$%&'()*+,-./:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^_`abcdefghijklmnopqrstuvwxyz{|}~";
  ...
  // Add '\0' to remember length
  *(passwd + plen) = '\0';
  // Fill remaining bytes with random data
  // This ensures that the same password will never encrypt to same ciphertext
  for (i = plen + 1; i < VALSIZE; i++)
    *(passwd + i) = choices[rand() % strlen(choices)];

  clen = encrypt((unsigned char *) passwd, VALSIZE, NULL, 0, enc_key, iv, ciphertext, tag);
  ciphertext[clen] = '\0';
    ...
    unsigned char *hmac_key )
{
  ...
  /* (3) Decrypt password */
  plen = decrypt(ciphertext, VALSIZE, NULL, 0, tag, enc_key, iv, *passwd);
...
char* get_replacement_choices(char c)
{
  if (isalnum(c))
    return "|\\/";
  else
    ...
    size_t *pwdlen)
{
  ...
  *passwd = (char *)malloc(VALSIZE);
  ...
  while ( guessNumber < MIN_GUESS_NUMBER ) {
    ...
    if (cnt == *pwdlen - 2) cnt = 1;
    else if (cnt == *pwdlen - 1) cnt = 0;

    char* choices = get_replacement_choices(*(*passwd + cnt));
    int idx = rand() % strlen(choices);
    *(*passwd + cnt) = choices[idx];
    cnt += 2;
  ...
  for (i = 0; i < KVS_BUCKETS; i++) {
    ...
    while ( kvle != NULL ) {
      ...
      if (enc_key) {  /* Dump decrypted value */
        ...
        BIO_dump_fp (fptr, (const char *)av->value, VALSIZE);
